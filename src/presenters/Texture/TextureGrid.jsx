...
class TextureGrid extends Component {
  ...
  generateImages({ texture, tilesWide, tilesHigh}) {
    ...
    const promises = range(tilesWide).map((row, x) =>
      Promise.all(range(tilesHigh).map((col, y) => {
        const cx = x * imageWidth;
        const cy = y * imageHeight;
        return cropImage(dataUri, cx, cy, imageWidth, imageHeight).then((croppedImage) => {
          return croppedImage;
        });
      }))
    );
  ...
  handleDrag(image, key, ev) {
    ev.dataTransfer.setData('style', JSON.stringify({
      image,
      key,
    }));
  }
  ...
  render() {
    ...
    const components = this.state.images.map(
      (imageRow, x) => imageRow.map(
        (image, y) => {
